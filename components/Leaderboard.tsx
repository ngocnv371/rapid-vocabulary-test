    ...
    <div className="w-full max-w-2xl mx-auto p-4">
      <div className="mb-6">
        <select
          ...
          className="w-full bg-gray-800 p-4 rounded-lg shadow-lg border border-purple-500/30 text-white focus:ring-purple-500 focus:border-purple-500 text-lg font-semibold"
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg border border-purple-500/30 text-center">
            <p className="text-gray-400 text-lg">No scores yet for this category. Be the first!</p>
          </div>
        ) : (
          scores.map((s, index) => (
            <div key={s.id || index} className="bg-gray-800 p-4 sm:p-6 rounded-lg shadow-lg border border-purple-500/30 hover:bg-gray-700/50 hover:border-purple-500 transition-all duration-300 ease-in-out">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-white font-semibold text-lg truncate" title={s.profiles?.name ?? s.profiles?.email ?? ''}>
                      {formatPlayerName(s.profiles?.name, s.profiles?.email)}
                    </p>
                    <div className="flex flex-col sm:flex-row sm:gap-4 text-sm text-gray-400">
                      <span>{categories.find(c => c.id === s.category)?.name}</span>
                ...
                <div className="flex-shrink-0">
                  <span className="text-purple-300 font-bold text-2xl">{s.score}</span>
                </div>
              </div>
            </div>
          ))
