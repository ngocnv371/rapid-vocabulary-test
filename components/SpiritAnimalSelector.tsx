...
const AnimalCard: React.FC<{ animal: Animal; onClick: () => void; }> = ({ animal, onClick }) => {
    return (
        ...
        >
            <span className="text-5xl sm:text-6xl mb-4" role="img" aria-label={`${animal.name} emoji`}>{animal.icon}</span>
            <h3 className="text-base sm:text-xl font-bold text-white tracking-wide leading-tight">{animal.name}</h3>
        </button>
    );
};


export default function SpiritAnimalSelector({ onSelect }: SpiritAnimalSelectorProps) {
  return (
    <div className="w-full max-w-3xl mx-auto p-4">
      ...
      <p className="text-lg text-center text-gray-300 mb-10">
        Pick the animal that speaks to your soul and start your vocabulary journey! 🌟
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-6">
        {spiritAnimals.map((animal) => (
          <AnimalCard key={animal.id} animal={animal} onClick={() => onSelect(animal)} />
        ))}
      </div>
    </div>
  );
